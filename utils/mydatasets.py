...
def get_dataset_iter(args, data_name = "MR"):
    ...
    elif data_name == "BabyName":
        train, test = BabyName.splits(TEXT, LABEL)
    else:
...
class BabyName(BasicDataset):
    """
        BabyName dataset
    """
    dirname = "./data/babyname"
    @staticmethod
    def sort_key(ex):
        return len(ex.text)
    ...
    def __init__(self, text_field, label_field, path=None, examples=None, **kwargs):
        ...
        if examples is None:
            path = self.dirname if path is None else path
            examples = []
            with open(os.path.join(path, 'girl_name.txt'), errors='ignore') as f:
                examples += [
                    data.Example.fromlist([line, 'girl'], fields) for line in f]
            with open(os.path.join(path, 'boy_name.txt'), errors='ignore') as f:
                examples += [
                    data.Example.fromlist([line, 'boy'], fields) for line in f]
        super(BabyName, self).__init__(examples, fields, **kwargs)
