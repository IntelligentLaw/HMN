...
def parse_train_from_file(file_path, label1_name = "parent_class", label2_name="laws"):
    with open(file_path, 'r') as f:
        line = f.read().rstrip()
        text, textlens, label1, label2, train_label = parse_line(line, label1_name, label2_name)
        return text, textlens, label1, label2, train_label
...
def parse_line(line, label1_name, label2_name):
    ...
    parent = json_line['parentIds']
    parentlens = json_line['parent_len']
    # parent_class = json_line['parent_class']
